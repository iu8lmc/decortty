...
TRANSLATIONS = {
    ...
    "The radio will not give audio to a bound client — reconnecting as a GUI station":
        "O rádio não dá áudio a um cliente ligado — volto a ligar-me como estação GUI",
    "TRANSMITTING":
        "A TRANSMITIR",
    ...
    "The audio comes from a sound card: the radio is not\nunder our control. Change band with the dial.":
        "O áudio vem de uma placa de som: o rádio não está sob o\nnosso controlo. Mude de banda no botão.",
    "The serial port is held by another program, so the\nradio takes no commands. Close it and reconnect.":
        "A porta série está ocupada por outro programa e o rádio\nnão aceita comandos. Feche-o e volte a ligar.",
    "%1 — RTTY at %2 MHz":
        "%1 — RTTY em %2 MHz",
    "Data on the upper sideband: the mode this\ndecoder is written for.":
        "Dados na banda lateral superior: o modo para o qual este\ndescodificador foi escrito.",
    "Data on the lower sideband. The tones come out\nreversed — REV puts them back.":
        "Dados na banda lateral inferior. Os tons saem invertidos\n— REV põe-nos no lugar.",
    "Voice sideband. RTTY is copied just the same,\nbut the radio's filter is wider than it needs.":
        "Banda lateral da fonia. O RTTY copia-se na mesma, mas o\nfiltro do rádio é mais largo do que precisa.",

    "Drag to move the radio; the wheel tunes by 1 kHz —\n100 Hz with Shift, 10 kHz with Ctrl.":
        "Arraste para mover o rádio; a roda sintoniza de 1 kHz —\n100 Hz com Shift, 10 kHz com Ctrl.",
    ...
    "Binds to the station already on the radio when there\nis one, and takes the GUI role when the radio is free.":
        "Liga-se à estação já ligada quando existe, e assume o\npapel de GUI quando o rádio está livre.",
    "GUI STATION":
        "ESTAÇÃO GUI",
    "Owns its own slices, so frequency and tuning always\nwork — at the cost of one of the two MultiFlex seats.":
        "Possui as suas próprias slices, por isso frequência e\nsintonia funcionam sempre — à custa de um lugar MultiFlex.",
    "SECOND CLIENT":
        "CLIENTE SECUNDÁRIO",
    "Reads the slices of the station that already has the\nradio, without taking a seat. If the radio shows none,\nDecoRTTY goes back to being a GUI station by itself.":
        "Lê as slices da estação que já tem o rádio, sem ocupar\nlugar. Se o rádio não mostrar nenhuma, o DecoRTTY volta\nsozinho a ser estação GUI.",
    "Takes effect at the next connection.":
        "Tem efeito na próxima ligação.",
    "Bound to %1 but the radio shows no slice — reconnecting as a GUI station":
        "Ligado a %1 mas o rádio não mostra nenhuma slice — a religar como estação GUI",
}
